...
def find_bestseller():
    ...
    try:
        best_seller = Pieza.select(Pieza.fabricante.alias('manufacturer'), fn.SUM(Pieza.n_sold).alias('total_pieces'))\
                        .group_by(Pieza.fabricante).order_by(SQL('total_pieces').desc()).limit(1)

        fabricante = Fabricante.select().where(Fabricante.id == best_seller[0].manufacturer)

        print(f'El fabricante más vendido es {fabricante[0].name}.\n'
              f'En total ha vendido {best_seller[0].total_pieces} piezas.')
    except:
...
def find_worstsellers():
    ...
    try:
        best_seller = Pieza.select(Pieza.fabricante.alias('manufacturer'), fn.SUM(Pieza.n_sold).alias('total_pieces'))\
                        .group_by(Pieza.fabricante).order_by(SQL('total_pieces').asc()).limit(1)

        fabricante = Fabricante.select().where(Fabricante.id == best_seller[0].manufacturer)

        print(f'El fabricante menos vendido es {fabricante[0].name}.\n'
              f'En total ha vendido {best_seller[0].total_pieces} piezas.')
    except:
        print('Excepcion indeterminada')
        # Esta linea es para que te muestre el motivo del error por pantalla
